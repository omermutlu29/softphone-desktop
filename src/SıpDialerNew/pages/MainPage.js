...
export default class MainPage extends Component {
  ...
  listenEvents = async () => {
    if (this.state.ua) {
      ...
      this.state.ua.on('newRTCSession', async (data) => {
        await save(data,this.props.updateRecords);
        ...
        if (data.session.direction === 'incoming') {
          ...
          data.session.on('ended', async (e) => {
            if (data.session) {
              await updateCall(data.session.id,data.session,this.props.updateRecords);
        ...
        if (data.session.direction === 'outgoing') {
          ...
          data.session.on('failed', async (e) => { // Your code here});
            if (data.session) {
              //updateCall(data.session.id,data.session);
              await this.sessionFail(e, data.session)
            }
          })
  ...
  numberButtonPressHandler = (event) => {
    if (this.state.activeSession && this.state.activeNumpad === '') {
      var tones = (event.target.getAttribute('data-id'))
      var options = {
        'duration': 160,
        'interToneGap': 1200,
        'extraHeaders': []
    ...
    this.setState({ callTo: this.state.callTo + (event.target.getAttribute('data-id')) })
  }
  ...
  holdSession = async () => {
    if (this.state.activeSession) {
      ...
      await this.setState({ activeSession: null, openPending: false })
  ...
  render() {
    return (
      <>
        {
          !this.state.activeSession ? <div className='head'>
            <div className='net'>
              <span></span>
              <span></span>
              <span></span>
            ...
            <div className='connect'>
              ...
              <span>{this.state.isRegistered ? 'Bağlandı' : 'Bağlanamadı'}</span>
            </div>
          </div> : null
        }
          ...
          this.state.activeSession && this.state.activeSession.isEstablished() ?
            this.state.numpadShow ?
              <Numpad
                ...
                deleteCallTo={this.deleteCallTo}
        ...
        </div>
        {
          this.state.pendingSessions.length !== 0 ?
            <div className='paused'>
              <div onClick={() => this.openPendings()} className='pause-menu-btn'>
                <img src='img/call-paused.gif' />
              </div>
              {
                this.state.pendingSessions.map((session, index) => (
                  <PendingSession
                    ringStop={this.ringStop}
                    session={session}
                    sessionEnd={this.sessionEnd}
                    sessionFail={this.sessionFail}
                    activeSession={this.state.activeSession}
  ...
  openPendings = async () => {
    this.setState({ openPending: !this.state.openPending }, () => {
      (this.state.openPending) ? $('.paused').css({ 'right': '0px' }) : $('.paused').css({ 'right': '-90px' })
    })
  }
